...
class TestMeasurementDispatcher:
    def test_measure_adds_time_entries_and_handles_sensor_errors(self, mocker):
        ...
        factory.create_sensor.side_effect = lambda name: {
            "foo": foo,
            "bar": bar,
            "bad": bad,
        }[name]
        ...
        time_values = [
            42.0,  # DataBuilder timestamp
            100.0,
            100.05,  # bar duration -> 50.0 ms
            200.0,
            200.10,  # foo duration -> 100.0 ms
            300.0,
            300.20,
        ]  # bad duration pair won't be used because it raises
        ...
        time_entries = [
            ...
            if e["tags"]["type"] == "time" and e["tags"]["unit"] == "ms"
        ]
...
class TestSensorFactory:
    def test_create_sensor_injects_supported_deps_and_reuses_i2c_bus(self, mocker):
        ...
        create_bus = mocker.patch(
            "klimalogger.measurement.create_i2c_bus", autospec=True
        )
    ...
    def test_i2c_bus_failure_is_handled_and_None_is_injected(self, mocker):
        cfg = configparser.ConfigParser()
        mocker.patch(
            "klimalogger.measurement.create_i2c_bus", side_effect=RuntimeError("no i2c")
        )
