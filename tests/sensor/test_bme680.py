...
from klimalogger.sensor.bme680 import BME680Sensor
...
def sensor_module():
    ...
    with patch(
        "klimalogger.sensor.bme680.adafruit_bme680", autospec=True
    ) as mock_module:
        yield mock_module
...
def test_measure_adds_all_fields_and_updates_measurements(
    uut, sensor_module, data_builder, temp_calc, pressure_calc
):
    ...
    assert (
        "relative humidity",
        "%",
        round(measured_relative_humidity, 2),
