...
def sensor_module():
    ...
    with patch(
        "klimalogger.sensor.bmp3xx.adafruit_bmp3xx", autospec=True
    ) as mock_module:
        yield mock_module
...
def test_measure_adds_pressure_and_sea_level_pressure(
    uut, sensor_module, data_builder, pressure_calc
):
    ...
    slp_entries = [
        e for e in data_builder.data if e["tags"]["type"] == "sea level pressure"
    ]
